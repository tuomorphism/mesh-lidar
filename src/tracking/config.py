...
class TrackingConf:
    ...
    min_points: int = 3  # min points to birth a new track
    min_confirm_hits: int = 2
    tentative_prune: int = 3

    # gating (used on top of Mahalanobis)
    max_dist: float = 5.0  # gating in meters for Euclidean XY
    w_shape: float = 0.05  # weight on relative size deviation
    shape_gate: float = 2.0  # max allowed avg relative size mismatch
    ...
    r_pos: float = 0.3
    r_yaw: float = np.deg2rad(8.0)

    # Mahalanobis^2 threshold (for 3D measurement: px, py, yaw)
    maha_thresh: float = 16.0

    # CRTV approximation
    straight_line_eps = 1e-5
